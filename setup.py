...
def handle_http_requirements(install_reqs):
    requirements = []
    for _req in install_reqs:
        try:
            req = _req.requirement
        except AttributeError:
            req = _req.req
        pkg_str = req
        if 'http' in req:
            pkg_name = req.rsplit('#egg=')[-1]
            pkg_str = f"{pkg_name} @ {req}"
        requirements.append(pkg_str)
    return requirements


requirements = handle_http_requirements(install_reqs)
